...
$sql = "SELECT score FROM busi where name='$name'";
$result = $conn->query($sql);
$row = $result->fetch_assoc();
$bestscore= $row['score'];

$sql = "SELECT * FROM busi order by score desc LIMIT 3";
$result = $conn->query($sql);
...
$score4=$row[1];
	$conn->close();
	echo "$bestscore";?>;
						e.canvas.fillStyle("#90EE90").fillScreen().fillStyle("#FFF").drawString(l.moduleName, 0, u + 110, e.graphics.VCENTER, !1, null, null, "50px 微软雅黑").drawImage("btns1", 0, 106, 480, 7, i - 480 >> 1, u + 140, 480, 7).drawImage("btns1", 0, 106, 480, 7, i - 480 >> 1, u + 555, 480, 7).fillStyle("#000").drawString(a(l.time, "秒"), 0, u + 340, e.graphics.VCENTER, !1, null, null, "60px 微软雅黑").fillStyle("#000").drawString("最佳:" + a(l.bestTime, "秒"), 0, u + 400, e.graphics.VCENTER, !1, null, null, "30px 微软雅黑").fillStyle("#000").drawString("距离好基友<?php echo "$first"?>仅差"+a(<?php $i=$score4-$bestscore;echo "$i"?>,"秒"),0, u + 440, e.graphics.VCENTER, !1, null, null, "30px 微软雅黑"),
					...
					var l = {
						...
						h = function() {
							...
							l.time > l.bestTime && e.localStorage.setItem("NotDieAnyoneBestTime" + l.module, l.time.toString()),
								dp_submitScore(l.module, l.time * (l.module - 1)),	$.ajax({url:'post.php?id=busi&score='+l.bestTime,type:'get',data:'',async: false,success:''})
						}
				}
			}
		}),
